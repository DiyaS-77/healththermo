...
#from libraries.bluetooth.bluez_le import BluetoothDeviceManager
...
class TestApplication(QWidget):
    ...
    '''def create_gatt_server_ui(self):
        ...
        title_label = QLabel("<b>GATT Server</b>")
        title_label.setFont(QFont("Segoe UI", 12, QFont.Weight.Bold))
        layout.addWidget(title_label)
        service_label = QLabel("Select Service:")
        service_label.setFont(bold_font)
        service_label.setStyleSheet(styles.color_style_sheet)
        ...
        self.service_combobox.setCurrentText("Battery Service")  # Default selection
        layout.addWidget(self.service_combobox)
        self.start_service_button = QPushButton("Start Server")
        self.start_service_button.clicked.connect(self.bluetooth_device_manager.create_gatt_server)
        self.start_service_button.setStyleSheet(styles.bluetooth_profiles_button_style)
        layout.addWidget(self.start_service_button)
        layout.addStretch(1)
        widget = QWidget()
        widget.setLayout(layout)
        ...
        title_label.setFont(QFont("Segoe UI", 12, QFont.Weight.Bold))
        layout.addWidget(title_label)
        service_label = QLabel("Registered Services:")
        service_label.setFont(bold_font)
        layout.addWidget(service_label)
        self.register_service = QListWidget()
        self.register_service.setFixedHeight(100)
        layout.addWidget(self.register_service)
        buttons_layout = QHBoxLayout()
        self.add_service_button = QPushButton("Add Service")
        self.add_service_button.setStyleSheet(styles.bluetooth_profiles_button_style)
        #self.add_service_button.clicked.connect(self.add_service)
        self.add_characteristic_button = QPushButton("Add Characteristic")
        self.add_characteristic_button.setStyleSheet(styles.bluetooth_profiles_button_style)
        #self.add_characteristic_button.clicked.connect(self.add_characteristic)
        buttons_layout.addWidget(self.add_service_button)
        buttons_layout.addWidget(self.add_characteristic_button)
    ...
    '''def select_pbap_single_save_location(self):
        """Opens a dialog to select save path for single contact."""
        options = QFileDialog.Option.DontUseNativeDialog
        file_path, _ = QFileDialog.getSaveFileName(
            None,
            "Select Save Location for Contact",
            "/root/Documents/tmp/phone.vcf",
            "vCard Files (*.vcf);;All Files (*)",
            options=options
        )
        if file_path:
            self.pbap_single_path_input.setText(file_path)

    def select_pbap_save_location(self):
        """Opens a file dialog to select where to save all PBAP contacts."""
        options = QFileDialog.Option.DontUseNativeDialog
        file_path, _ = QFileDialog.getSaveFileName(
            None,
            "Select Save Location",
            "/root/Documents/tmp/Contacts.vcf",
            "vCard Files (*.vcf);;All Files (*)",
            options=options
        )
        if file_path:
            self.pbap_all_path_input.setText(file_path)'''
    ...
    def get_profile_selection_dialog(self):
        ...
        return dialog, all_profiles, a2dp_checkbox, opp_checkbox, pbap_checkbox, sink_radio

    def handle_connect_clicked(self, device_address):
        dialog, all_profiles, a2dp_checkbox, opp_checkbox, pbap_checkbox, sink_radio = self.get_profile_selection_dialog()
        if not dialog.exec():
            return
    ...
    '''def handle_connect(self, device_address, load_profiles):
        dialog, all_profiles, a2dp_checkbox, opp_checkbox, sink_radio = self.get_profile_selection_dialog()
        if not dialog.exec():
            return
        ...
        for profile, role in self.selected_profiles.items():
            if not self.connect_profile(device_address, profile, role):
        ...
        if not failed_profiles:
            ...
            QMessageBox.warning(self, "Connection Result", "\n".join(message_parts))
    ...
    def connect_profile(self, device_address, profile, role):
        if device_address not in self.device_profiles:
            self.device_profiles[device_address] = []

        if profile == 'all':
            if not self.bluetooth_device_manager.connect(device_address):
                return False

            connected_uuids = self.bluetooth_device_manager.get_connected_profile_uuids(device_address)
            a2dp_connected = (constants.profile_uuids["A2DP Sink"] in connected_uuids or
                              constants.profile_uuids["A2DP Source"] in connected_uuids)
            opp_connected = constants.profile_uuids["OPP"] in connected_uuids

            if not a2dp_connected or not opp_connected:
                return False
        ...
        if profile == 'a2dp':
            ...
            if self.bluetooth_device_manager.connect_profile(device_address, profile_uuid=uuid):
                self.device_profiles[device_address].append("A2DP")
                return True
            return False
    ...
    '''def create_gatt_server_ui(self):
        ...
        self.service_combobox.setCurrentText("Battery Service")
        layout.addWidget(self.service_combobox)

        self.service_info_label = QLabel("UUID: 0x180F\nCharacteristic: Battery Level (0x2A19)")
        self.service_info_label.setFont(QFont("Arial", 9))
        self.service_info_label.setStyleSheet("color: gray;")
        layout.addWidget(self.service_info_label)

        def update_service_info():
            selected = self.service_combobox.currentText()
            if selected == "Battery Service":
                self.service_info_label.setText("UUID: 0x180F\nCharacteristic: Battery Level (0x2A19)")

        self.service_combobox.currentIndexChanged.connect(update_service_info)
        self.start_service_button = QPushButton("Start Server")
        self.start_service_button.setStyleSheet(styles.bluetooth_profiles_button_style)
        ...
        self.stop_service_button.setStyleSheet(styles.bluetooth_profiles_button_style)
        self.stop_service_button.clicked.connect(self.handle_stop_server)
        layout.addWidget(self.stop_service_button)

        self.adv_toggle_button = QPushButton("Start Advertising")
        self.adv_toggle_button.setStyleSheet(styles.bluetooth_profiles_button_style)
        self.adv_toggle_button.clicked.connect(self.toggle_advertising)
        layout.addWidget(self.adv_toggle_button)
        # --- Show Connected Device Button ---
        self.show_connected_button = QPushButton("Show Connected Device")
        self.show_connected_button.setStyleSheet(styles.bluetooth_profiles_button_style)
        self.show_connected_button.clicked.connect(self.show_connected_device)
        layout.addWidget(self.show_connected_button)

        # Label to display connected device
        self.connected_device_label = QLabel("Connected Device: None")
        self.connected_device_label.setFont(QFont("Arial", 9))
        self.connected_device_label.setStyleSheet("color: gray;")
        layout.addWidget(self.connected_device_label)

        status_title = QLabel("Status:")
        status_title.setFont(bold_font)
        status_title.setStyleSheet(styles.color_style_sheet)
       # layout.addWidget(status_title)

        self.server_status_label = QLabel("Server: Stopped")
        self.server_status_label.setFont(QFont("Arial", 9))
       # layout.addWidget(self.server_status_label)

        self.adv_status_label = QLabel("Advertising: Stopped")
        self.adv_status_label.setFont(QFont("Arial", 9))
        #layout.addWidget(self.adv_status_label)
    ...
    def handle_advertising(self):
        #items = ["Battery Service (0x180F)"]
        items = [
            "Battery Service (0x180F)",
            "Scan Parameters Service (0x1813)",
            "Find Me Service (0x1802)",
        ...
        if service_choice.startswith("Battery"):
            ...
            uuid = "1813"
        elif service_choice.startswith("Find Me"):
            uuid = "1802"
    ...
    def create_gatt_server_ui(self):
        ...
        self.service_combobox = QComboBox()
        #self.service_combobox.addItems(["Battery Service"])
        self.service_combobox.addItems([
            "Battery Service (0x180F)",
            "Scan Parameters Service (0x1813)",
            "Find Me Service (0x1802)",
